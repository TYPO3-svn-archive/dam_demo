...
class tx_damdemo_sv2 extends t3lib_svbase {
	var $prefixId = 'tx_damdemo_sv2';		// Same as class name
	var $scriptRelPath = 'sv2/class.tx_damdemo_sv2.php';	// Path to this script relative to the extension dir.
			...
			foreach ($input as $line) {
				...
				if ($value) { // ignore empty lines headers and emtpy entries
					...
					switch ($name) {
						...
						break;
						case 'childFile':
								// a dependent file to this file should be indexed

							if(is_file($value)) {
								$childMetaPreset = array();
								$childMetaPreset['fields'] = array();
								$childMetaPreset['fields']['description'] = 'This is a child of '.$this->out['fields']['title'];

								$this->out['childs'][] = array(
															'pathname' => $value,
															'fileStorageType' => 'moveToInternal',
															'metaPreset' => $childMetaPreset,
														);
							}
						break;
					}
	...
	function _createTestFileContent() {
			...
			// creating child test files
		$tempfile1 = t3lib_div::tempnam('tx_damdemo_sv2');
		$tempfile2 = t3lib_div::tempnam('tx_damdemo_sv2');

		t3lib_div::writeFileToTypo3tempDir($tempfile1, md5(microtime()));
		t3lib_div::writeFileToTypo3tempDir($tempfile2, md5(microtime()));
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam_demo/sv2/class.tx_damdemo_sv2.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam_demo/sv2/class.tx_damdemo_sv2.php']);
}
